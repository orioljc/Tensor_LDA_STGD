...
MatrixXd compute_empirical_T3_point(SparseMatrix<double> whitened_data, VectorXd y_mean, int datapoint){
	int nx = (int)whitened_data.cols();
	//printf("nx = %d\n", nx);
	double shift12 = (alpha0 + 1.0)*(alpha0 + 2.0) / 2.;
	double shift01 = -alpha0 *(alpha0 + 1.0) / 2.;
	double shift00 = alpha0*alpha0;

	MatrixXd emp_T3_whitened_point = MatrixXd::Zero(KHID, KHID * KHID);

	VectorXd y = whitened_data.col(datapoint);
	...
	temp4.noalias() = shift00 * (y_mean * y_mean.transpose());
	for(int i=0; i < KHID; i++){
		emp_T3_whitened_point.block(0,i*KHID, KHID, KHID).noalias() += temp0*y(i);
		emp_T3_whitened_point.block(0,i*KHID, KHID, KHID).noalias() += temp1*y_mean(i);
		emp_T3_whitened_point.block(0,i*KHID, KHID, KHID).noalias() += temp2*y(i);
		emp_T3_whitened_point.block(0,i*KHID, KHID, KHID).noalias() += temp3*y(i);
		emp_T3_whitened_point.block(0,i*KHID, KHID, KHID).noalias() += temp4*y_mean(i);			
	}
	return emp_T3_whitened_point;
}
