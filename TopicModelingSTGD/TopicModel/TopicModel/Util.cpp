...
//Choose only words that appear more than 3 times in the corpus?? 
void accumulate_M_mul_S(SparseMatrix<double> Gx_a, SparseMatrix<double> RandomMat, \
	...
	//Calculate the empirical 2nd order moment M2
    ...
    //not count documents with less that 3 words??
    //Lengths contains the number of words of each document
...
void Compute_M3_topic(MatrixXd whitenedData, VectorXd whitenedMean, VectorXd Lengths, MatrixXd &Ta, int t){
	unsigned int k = KHID;
	unsigned int kstart = 0;	unsigned int kend = k-1;	assert(kend < k);
	unsigned int krange = 1 + (kend - kstart);
	

	for (int examples = 0; examples < NX[t]; ++examples){
...
double tensorDecom_batchALS(MatrixXd T, VectorXd & lambda, MatrixXd & A_new){
	...
	while (true)
	{
        ...
        if (iteration > MINITER )
        {
            ...
            // cout << "T_est (INSIDE):\n" << T_est << endl;
            // cout << "T (INSIDE):\n" << T << endl;
            // cout << "error = (T_est - T).norm() (INSIDE):\n" << (T_est - T).norm() << endl;
...
//                cout << "err: " << error << endl;
            ...
            {
                fail = 1;
             //A_new, B_new and C_new are not equal...
             //    cout << "A_new (INSIDE):\n" << A_new << endl;
             //    cout << "B_new (INSIDE):\n" << B_new << endl;
             //    cout << "C_new (INSIDE):\n" << C_new << endl;
            	// tensorReconstruct(T_est, A_new, A_new, A_new, lambda);
             //    cout << "T_est (INSIDE):\n" << T_est << endl;
            	// tensorReconstruct(T_est, A_new, B_new, C_new, lambda);
             //    cout << "T_est (INSIDE):\n" << T_est << endl;                
                break;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void tensorDecom_alpha0_topic(SparseMatrix<double> D_a_mat, VectorXd D_a_mu, VectorXd Lengths, VectorXd &lambda, MatrixXd & phi_new, int t)
{
	...
	// find norm terms
	VectorXd OnesPseudoX = VectorXd::Ones(NX[t]);
	...
	while (true)
	{
		...
		for (int index_k = 0; index_k < KHID; index_k++){
			...
			VectorXd CorreRewd_0 = para_main * tensor_form_main_topic(Gx_aNorm0, Gx_aNorm1, Gx_aNorm2, Gx_aSquareNorm, Gx_aCubNorm, curr_eigenvec, t);
...
VectorXd tensor_form_main_topic(SparseMatrix<double> Gx_aNorm0, SparseMatrix<double> Gx_aNorm1, \
	 SparseMatrix<double> Gx_aNorm2, SparseMatrix<double> Gx_aSquareNorm, SparseMatrix<double> Gx_aCubNorm, VectorXd curr_eigenvec, int t){
	// T(.,vi,vi) = 1/n sum [1/l(l-1)(l-2)]  { (ct'*vi)^2 ct - }
	// find norm terms
	VectorXd D_b_f = Gx_aNorm1.transpose() * curr_eigenvec; // vector of dimension NX[t]
	VectorXd D_c_f = Gx_aNorm2.transpose() * curr_eigenvec;
	VectorXd D_bc_f = D_b_f.cwiseProduct(D_c_f);		// vector of dimensionn NX[t], coefficient for each sample... 
	// shifts1
	MatrixXd eigenMat = curr_eigenvec.replicate(1, Gx_aSquareNorm.cols()); //D_a_mat.cols() = NX[t];
...
// void tensorDecom_alpha0_online(SparseMatrix<double> D_a_mat, VectorXd D_a_mu, SparseMatrix<double> D_b_mat, VectorXd D_b_mu, SparseMatrix<double> D_c_mat, VectorXd D_c_mu, VectorXd &lambda, MatrixXd & phi_new, int t)
// {
// 	double error;
// 	MatrixXd A_random(MatrixXd::Random(KHID, KHID));
// 	MatrixXd phi_old;
